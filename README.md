...
c2w                 # 改写下两个单词
ct(                 # 改写到小括号前
x                   # 删除当前字符，前面可以接数字，Nx 代表删除 N 个字符
X                   # 向前删除字符
...
yiw                 # 复制当前单词
Nyy                 # 复制光标下 N 行内容
ddp                 # 剪切当前行并粘贴，即先删除当前行复制到寄存器，并粘贴
v0                  # 选中当前位置到行首
v$                  # 选中当前位置到行末
viw                 # 选中当前单词
vib                 # 选中小括号内的内容
...
```bash
u                   # 撤销命令，可以组合，例如 Nu，N 是任意整数，表示撤销 N 步操作，下同（u: undo）
U                   # 撤销整行操作
...
:%s/^M$//g          # 删除 Vim 文件中显式的 ^M 符号，即操作系统换行符问题
```
...
**多行注释**
```bash
Ctrl+v              # 进入命令行模式，按 Ctrl+v 进入可视模式，然后按 j 或者 k 字母键选中多行，把需要注释的行标记起来
I                   # 按大写字母 I 字母键，再插入注释符，例如 #、//
<Esc>               # 按 <Esc> 退出键就会全部注释了
```

**取消多行注释**

```bash
Ctrl+v              # 进入命令行模式，按 Ctrl+v 进入可视模式，按字母 l 字母键横向选中列的个数，例如 #、//，需要选中 2 列
j 或 k              # 按字母 j 或者 k 键选中注释符号
d                   # 按 d 字母键就可全部取消注释
```
...
Ctrl+O              # 跳转到上一个位置
Ctrl+I              # 跳转到下一个位置
Ctrl+^              # 跳转到 alternate file，即当前窗口的上一个文件
%                   # 匹配跳转到对应的 {} () []
gd                  # 跳转到局部定义，即光标下的单词的定义
gD                  # 跳转到全局定义，即光标下的单词的定义
gf                  # 打开名称为光标下文件名的文件
...
:e[dit] {filename}  # 打开文件并编辑，通过文件的绝对或相对路径打开文件，Tab 制表键补全路径
...
:vs {filename}      # 左右纵向切分窗口并在新窗口打开文件 filename（vs: vertical split）
:split              # 将当前窗口再复制一个横向窗口出来，内容同步，游标可以不同
:vsplit             # 将当前窗口再复制一个纵向窗口出来，内容同步，游标可以不同
Ctrl+W              # 切换到下一个窗口（Window） 
Ctrl+W s            # 上下横向切分窗口（s: split）       
Ctrl+W v            # 左右纵向切分窗口（v:vertical split）
Ctrl+W w            # 循环切换到下一个窗口 
Ctrl+W W            # 循环切换到上一个窗口 
Ctrl+W p            # 跳到上一个访问过的窗口 
Ctrl+W r            # 互换窗口（r: reverse）
Ctrl+W c            # 关闭当前窗口，但不能关闭最后一个窗口（c: close）
Ctrl+W q            # 退出当前窗口，如果是最后一个窗口，则关闭 vi（q: quit）
Ctrl+W o            # 只保留活动窗口，关闭其他窗口，等效于 :on[ly]（o: other）
Ctrl+W h            # 跳到左边的窗口
Ctrl+W j            # 跳到下边的窗口
Ctrl+W k            # 跳到上边的窗口
Ctrl+W l            # 跳到右边的窗口
...
Ngt                 # 切换到第 N 个标签页，例如 2gt 将会切换到第 2 个标签页
gt                  # 下一个标签页
gT                  # 上一个标签页
```
...
> **文档加密**，打开文件时会在屏幕左下角提示输入密码两次才可进行操作，保存文件退出后必须输入正常密码才能正确打开文件，否则会显示乱码。

```bash
vim -x {filename}	# 输入加密密码并再次确认密码。注意：不修改内容也要保存，否则密码设定不会生效
:X                  # 命令模式下输入加密密码并再次确认密码。注意：不修改内容也要保存，否则密码设定不会生效
:set key=密码		# 命令模式下输入加密密码并再次确认密码。注意：不修改内容也要保存，否则密码设定不会生效
```
...
- 先将光标移动到第一行。
- 在 Normal 模式下，按 q 字母键加一个字母开始录制。例如按下 qa，将该宏注册为 a。
- 按下 I 字母键在行首插入，在编辑模式按下 Tab 制表键。按 <Esc> 退出键返回到 Normal 模式。
- 按下 j 字母键将光标移动到下一行。
- 按下 q 字母键完成录制。
...
| 字母寄存器        | "a - "z 或 "A - "Z | {register}只能是一位的 26 个英文字母，从 a-z，A-Z 寄存器内容将会合并到对应小写字母内容后边 |
...
```bash
普通模式            # 按 <Esc> 退出键或 Ctrl+[ 进入，左下角显示文件名或为空
插入模式            # 按 i 字母键进入，左下角显示 --INSERT--
可视模式            # 按 v 字母键进入，左下角显示 --VISUAL--
替换模式            # 按 r 或 R 字母键开始，左下角显示 --REPLACE--
命令行模式          # 按 : 或者 / 或者 ? 开始
```
