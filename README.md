...
> 还有一种比替换更灵活的方式，它允许匹配到某个指定模式后执行指定的 Ex 命令，即 Vim 的 global 命令，其处理重复工作的效率极高。命令格式为：
...
> 高级的查找替换就要用到正则表达式。此处不进行过多阐述，详情查看下面的 正则表达式 命令的帮助文档。
...
\{,m}              # 表示匹配 0-m 个目标字符，即最多匹配 (m-1) 个目标字符，也可以不匹配 (0)
                   # ---------------- 表示【位置】的元字符 ---------------------
^                  # 表示匹配 输入字符串的开始位置 (行首)
$                  # 表示匹配 输入字符串的结束位置 (行尾)
\<                 # 表示匹配 单词词首
...
> Vim 的 diff 模式是依赖于 diff 命令的，所以首先保证系统中的diff命令是可用的。
