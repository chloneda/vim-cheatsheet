...
```
cit、dit、yit、vit，分别操作一对标签之间的内容，编辑 HTML、XML 很好用！
...
```bash
u                   # 撤销命令 (undo)，可以组合，例如 Nu, N 是任意整数，表示撤销 N 步操作，下同
U                   # 撤销整行操作
Ctrl+r              # 撤销上一次 u 命令(redo)
Ctrl+R              # 回退前一个命令
```
...
:Ctrl+r "寄存器名称      # 命令模式下（输入 Ctrl+r 后 Vim 会自动打出"寄存器引用符号）
...
| 逐级临时缓存寄存器 | "1 - "9 | 所有不带范围（‘(’，‘)’，‘{’，‘}’）、操作涉及1行以上的删除修改操作（x、s、d、c）的数据都会复制到逐级临时缓存寄存器，并在新的数据加入时，逐级先后推移。1 的数据复制到 2，2 的复制到 3，最后的 9 寄存器内容将会被删除 |
